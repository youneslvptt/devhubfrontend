...
function ChatPage() {
  ...
  const activeChannel = useMemo(
    () => channels.find((channel) => channel._id === activeChannelId) ?? channels[0],
  ...
  useEffect(() => {
    if (!channels.length || !activeChannelId) return;
    if (search.channelId === activeChannelId) return;
  ...
  useEffect(() => {
    ...
    const onOnlineUsers = (payload) => {
      // Backend now sends array of { userId, username } objects
      const users = Array.isArray(payload) ? payload : [];
      setOnlineUsers(users);
    };

    const onUserJoined = (onlineUser) => {
      if (!onlineUser?.username) return;
      setOnlineUsers((current) =>
        current.some((entry) => entry.username === onlineUser.username)
          ? current
          : [...current, onlineUser]
      );
    };
    ...
    // ✅ Socket error listener – shows backend rejections
    const onError = (errorMsg) => {
      console.error("[chat] socket error:", errorMsg);
    };
    ...
    if (user) {
      socket.emit("userOnline", user._id ?? user.id ?? user.username);
    }
