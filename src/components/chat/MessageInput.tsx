...
import type { MessageAttachment } from "./MessageList";
...
async function uploadFile(file: File): Promise<MessageAttachment> {
  ...
  try {
    ...
    const url: string | undefined = data?.url ?? data?.fileUrl ?? data?.path;
...
export function MessageInput({ channelName, onSend, onTyping, disabled }: Props) {
  ...
  return (
    <form
      ...
      onDrop={onDrop}
      className={`relative border-t border-border bg-background px-6 py-4 transition-colors ${
            ...
            {pending.map((p) => (
              ...
              >
                ...
                <button
                  ...
                  className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-background/80 text-muted-foreground opacity-80 transition-all hover:bg-destructive hover:text-destructive-foreground"
